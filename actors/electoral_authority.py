...
class ElectoralAuthority:
    def __init__(self, sa_public_key):
        ...
        self.bulletin = BulletinBoard(self.private_key)

        private_pem = private_key_to_pem(self.private_key)
        self.private_key_length = len(private_pem)
    ...
    def _publish_parameters(self) -> None:
        atomic_write_json(PUBLIC_PARAMETERS_FILE, {
            "election_id": ELECTION_ID,
            "protocol_version": PROTOCOL_VERSION,
            "votazioni_inizio": VOTAZIONI_INIZIO,
            "votazioni_fine": VOTAZIONI_FINE,
            "opzioni_ammesse": ["SI", "NO"],
            "ae_public_key": public_key_to_pem(self.public_key),
            "sa_public_key": public_key_to_pem(self.sa_public_key),
            "trustees": N_TRUSTEES,
            "threshold": THRESHOLD,
            "hybrid_encryption": "Fernet + RSA-OAEP-SHA256",
            "signatures": "RSA-PSS-SHA256",
            "pseudonymous_ballot_signature": "RSA-PSS-SHA256",
            "bulletin_integrity": "SHA-256 hash-chain + Merkle root",
        })
    ...
    def _export_trustee_shares(self) -> None:
        ...
        atomic_write_json(TRUSTEE_SHARES_FILE, {
            ...
            "secret_length": self.private_key_length,
            "shares": shares,
            "nota": (
    ...
    def encrypt_vote(self, vote: str) -> HybridCiphertext:
        ...
        encrypted_ballot = Fernet(session_key).encrypt(canonical_json(payload))
    ...
    def receive_ballot(self, message: BallotMessage) -> Optional[SignedReceipt]:
        ...
        signed_receipt = SignedReceipt(
            ...
            signature=sign_json(self.private_key, receipt_data),
        )
        atomic_write_json(
    ...
    def scrutinio(self, authenticated_trustees: List[str]) -> Optional[dict]:
        if not self.bulletin.closed or not CLOSURE_FILE.exists():
            print("Le urne devono essere chiuse prima dello scrutinio")
            return None

        distinct = list(dict.fromkeys(authenticated_trustees))
        ...
        for username in distinct[:THRESHOLD]:
            index = int(username.replace("trustee", "")) - 1
            share = self.trustee_shares[index]
            if not self._verify_share_commitment(username, share):
                print(f"Commitment della quota non valido: {username}")
                return None
            shares.append(share)
        ...
        try:
            ...
            reconstructed_key = load_private_key(reconstructed_pem)
        except (ValueError, TypeError) as exc:
            print("Ricostruzione della chiave fallita:", exc)
            return None

        # Test che la chiave ricostruita corrisponda alla chiave pubblica ufficiale.
        reconstructed_public = public_key_to_pem(reconstructed_key.public_key())
        if reconstructed_public != public_key_to_pem(self.public_key):
            print("La chiave ricostruita non corrisponde alla chiave pubblica AE")
            return None
        ...
        for record in self.bulletin.records:
            try:
                payload = self._decrypt_ballot(record, reconstructed_key)
                if payload.get("election_id") != ELECTION_ID:
                    raise ValueError("Election ID errato")
                if payload.get("protocol_version") != PROTOCOL_VERSION:
                    raise ValueError("Versione errata")
                vote = payload.get("vote")
                if vote not in {"SI", "NO"}:
                    raise ValueError("Preferenza non ammessa")
                ...
                decrypted_ballots.append({
                    ...
                    "vote": vote,
                    "ballot_nonce": payload.get("ballot_nonce"),
                })
            except (
                ValueError,
                InvalidToken,
        ...
        winner = "SI" if si > no else "NO" if no > si else "PAREGGIO"
        ...
        result_data = {
            ...
            "totale_registrati": len(self.bulletin.records),
            "totale_validi": si + no,
            "si": si,
            "no": no,
            "invalidi": invalidi,
            "vincitore": winner,
            "hash_chain_valid": self.bulletin.verify_chain(),
            "bulletin_last_hash": self.bulletin.last_hash,
            "bulletin_merkle_root": self.bulletin.root(),
            "closure_hash": sha256_hex(
                canonical_json(closure_document["closure"])
            ),
        ...
        document = {
            "result": result_data,
            "signature": sign_json(reconstructed_key, result_data),
            "signature_algorithm": "RSA-PSS-SHA256",
            "ae_public_key": public_key_to_pem(self.public_key),
        }
        atomic_write_json(RESULT_FILE, document)
        return document

    def verify_receipt(self, receipt_file: Path) -> bool:
        try:
            ...
            record = self.bulletin.find(ballot_id)
            if record is None:
                return False
            if record.record_hash != receipt["record_hash"]:
                return False
    ...
    def universal_verify(self) -> bool:
        ...
        public_key = load_public_key(result_document["ae_public_key"])
        ...
        temp_board.ae_private_key = None
